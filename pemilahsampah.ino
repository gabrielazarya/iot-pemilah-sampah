...
const int inductiveSensorPin = 2;  // Pin sensor proximity induktif di ESP32

int ultrasonicTriggerPin = 12;  // Pin trigger sensor ultrasonik untuk sampah non-logam
int ultrasonicEchoPin = 14;  // Pin echo sensor ultrasonik untuk sampah non-logam
...
void setup() {
    ...
    pinMode(ultrasonicTriggerPin, OUTPUT);  // Set trigger pin for ultrasonic sensor as output
    pinMode(ultrasonicEchoPin, INPUT);  // Set echo pin for ultrasonic sensor as input
    pinMode(ultrasonicTriggerPinKapasitasLogam, OUTPUT);  // Set trigger pin for ultrasonic sensor as output
    pinMode(ultrasonicEchoPinKapasitasLogam, INPUT);  // Set echo pin for ultrasonic sensor as input
    pinMode(ultrasonicTriggerPinKapasitasNonLogam, OUTPUT);  // Set trigger pin for ultrasonic sensor as output
    pinMode(ultrasonicEchoPinKapasitasNonLogam, INPUT);  // Set echo pin for ultrasonic sensor as input
    ...
    client.setInsecure();  // Tambahkan ini untuk memastikan koneksi aman ke server Telegram
    ...
    delay(2000);
    lcd.clear();  // Membersihkan layar setelah delay
    ...
    servoPemilah.write(90);  // Initialize servo to default position
    
    servoPenutup.attach(servoPenutupPin);
    servoPenutup.write(0);  // Initialize servo to default position
}
...
void loop() {
    ...
    if (!metalDetected && nonMetalDetected) {
        ...
        // Buka servo logam
        delay(100);  // Delay setelah mencapai 180 derajat
        servoPemilah.write(180);
        delay(2000);  // Delay setelah mencapai 180 derajat
